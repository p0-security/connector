...
/**
 * Prefix stamped onto every user this connector provisions.
 *
 * Namespacing is the safety property this example takes seriously: it is what
 * lets `deleteUser` and `setPoliciesForUser` refuse to touch an account P0 did
 * not create. Keep an equivalent marker in your own connector — a prefix, a
 * group membership, a tag, a dedicated OU — and check it before every write.
 */
const P0_PREFIX = "p0_";

/** Maps the principal P0 sends to this application's own user identifier. */
const userIdFor = (principal: string): UserId => `${P0_PREFIX}${principal}`;

/**
 * Refuses any write against a user this connector did not provision.
 *
 * Throwing {@link ConnectorError} rather than a bare `Error` is what turns
 * this into a classified 412 that P0 can explain to an operator, instead of an
 * opaque 500.
 */
const assertP0Managed = (userId: UserId): void => {
  if (userId.startsWith(P0_PREFIX)) return;
  throw new ConnectorError({
    type: "validation_error",
    message: `${userId} was not provisioned by P0; this connector will not modify it`,
    payload: { userId, expectedPrefix: P0_PREFIX },
  });
};

/**
 * Builds the five actions P0 calls, once per request.
 *
